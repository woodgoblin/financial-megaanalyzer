...
class AIBDebitParser:
    ...
    def _identify_columns(self, pdf) -> dict | None:
        ...
        for page in pdf.pages:
            ...
            for word in words:
                ...
                if text == 'DEBIT' or (text.startswith('DEBIT') and '€' in word.get('text', '')):
                    debit_header = word
                elif text == 'CREDIT' or (text.startswith('CREDIT') and '€' in word.get('text', '')):
                    credit_header = word
                elif text == 'BALANCE' or (text.startswith('BALANCE') and '€' in word.get('text', '')):
                    balance_header = word
    ...
    def _extract_date_from_line(self, line_words: list, date_pattern: re.Pattern) -> str | None:
    ...
    def _process_opening_balance_line(self, line_words: list, line_text: str, 
                                     balance_range: tuple, date_pattern: re.Pattern) -> tuple[dict | None, str | None]:
        """
        ...
        return {
            ...
            'details': "OPENING BALANCE" if "OPENING" in line_text.upper() else "BALANCE FORWARD",
            'date': extracted_date,
        }, extracted_date

    def _find_transaction_amounts(self, line_words: list, debit_range: tuple, 
                                 credit_range: tuple, balance_range: tuple) -> tuple[list, float | None, str | None]:
        """
    ...
    def _collect_transaction_details(self, line_words: list, amount_word: dict, 
                                    current_date: str, date_pattern: re.Pattern,
                                    debit_range: tuple) -> str:
    ...
    def _collect_additional_info_from_lines(self, start_idx: int, sorted_y: list, lines: dict,
                                           date_pattern: re.Pattern, debit_range: tuple,
                                           credit_range: tuple, balance_range: tuple,
                                           footer_keywords: list, details_max_x: float,
                                           initial_reference: str | None, initial_balance: float | None) -> dict:
        """
        ...
        for j in range(start_idx + 1, min(start_idx + MAX_LOOKAHEAD_LINES, len(sorted_y))):
            next_y = sorted_y[j]
                ...
                if re.match(r'^\d+\.?\d{1,2}$', text):
                    try:
                        ...
                        if MIN_TRANSACTION_AMOUNT <= amt <= MAX_TRANSACTION_AMOUNT:
                            x_center = (word.get('x0', 0) + word.get('x1', 0)) / 2
                            if (debit_range[0] <= x_center <= debit_range[1] or 
                                credit_range[0] <= x_center <= credit_range[1]):
                                has_another_tx = True
                                break
                    except ValueError:
                        pass
            ...
            if 'original_currency' not in result['fx_info']:
                fx_match = re.search(r'(\d+\.?\d*)\s+([A-Z]{3})@\s*(\d+\.?\d+)?', next_line_text)
                if fx_match:
                    ...
                    if fx_match.group(3):
                        result['details_extension'] += f" {fx_match.group(1)} {fx_match.group(2)}@ {fx_match.group(3)}"
                    else:
                        result['details_extension'] += f" {fx_match.group(1)} {fx_match.group(2)}@"
            
            # Collect FX fee
            if 'fx_fee' not in result['fx_info']:
                fx_fee_match = re.search(r'INCL FX FEE\s+[E€]?(\d+\.?\d+)', next_line_text)
                if fx_fee_match:
                    result['fx_info']['fx_fee'] = float(fx_fee_match.group(1))
                    result['details_extension'] += " INCL FX FEE E" + fx_fee_match.group(1)
            ...
            if not re.match(r'^\d+\.?\d+$', next_line_text.strip()):
                ...
                details_words = [
                    w for w in next_line_words 
                    if w.get('x0', 0) < details_max_x
                ]
    ...
    def _extract_transactions_by_columns(self, pdf, column_bounds: dict) -> list[Transaction]:
        """
        ...
        prev_balance = None
        opening_balance_tx = None  # Store opening balance transaction to insert at start
        ...
        for page in pdf.pages:
            ...
            while i < len(sorted_y):
                ...
                if "BALANCE FORWARD" in line_text.upper() or "OPENING BALANCE" in line_text.upper():
                ...
                tx_amounts, balance_value, reference_value = self._find_transaction_amounts(
                ...
                for tx_type, amount, amount_word in tx_amounts:
                    ...
                    additional_info = self._collect_additional_info_from_lines(
                        i, sorted_y, lines, date_pattern, debit_range, credit_range,
                        balance_range, footer_keywords, details_max_x,
                        reference_value, balance_value
                    )
                    
                    # Combine details with extension
                    full_details = (details + additional_info['details_extension']).strip()
                    ...
                    transaction = Transaction(
                        ...
                        original_currency=additional_info['fx_info'].get('original_currency'),
                        original_amount=additional_info['fx_info'].get('original_amount'),
