...
def main():
    ...
    print(
        f"Continuous Period:    {analysis.summary.continuous_period_start} -> "
    ...
    print("-" * 100)
    print(
        f"{'File Name':<30} {'Start Date':<15} {'End Date':<15} {'Parser':<20} {'Modified':<20}"
    )
    print("-" * 100)
    ...
    for stmt in analysis.statements:
        ...
        if stmt.error:
            print(
                f"{stmt.file_name:<30} {'ERROR':<15} {'ERROR':<15} {parser_name:<20} {mod_str:<20}"
            )
            print(f"  -> {stmt.error}")
        else:
            print(
                f"{stmt.file_name:<30} {stmt.start_date:<15} {stmt.end_date:<15} {parser_name:<20} {mod_str:<20}"
            )
