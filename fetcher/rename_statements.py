...
def rename_statements(
    input_dir: Path, output_dir: Path = None, dry_run: bool = False
) -> dict:
    """
    ...
    for stmt in analysis.statements:
        ...
        new_filename = (
            f"{parser_sanitized}_from_{start_formatted}_to_{end_formatted}{extension}"
        )
...
def main():
    ...
    parser.add_argument(
        "input_dir", type=Path, help="Directory containing PDF statement files"
    )
    parser.add_argument(
        "-o",
        "--output",
